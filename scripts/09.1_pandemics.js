...
;
  ...
  for (let i = 0; i < 18; i++) {
    let r = paper.rect((paperWidth/22)*(i+1.5),800,paperWidth/22, paperHeight).attr({
      opacity:0,
      id: "nav"
    });
    r.hover(
      function(e) {
          ...
          drawCompMap(monthlyCountrySars, monthlyCovid[i].dateRounded, colors[3]);
          r.attr({opacity: 0});
...
function calculateMonths(){
    ...
    monthlyComp.push(...monthlySars,...monthlySwine,...monthlyEbola,...monthlyCovid);
...
function drawCompMap(data, month, col) {
  ...
  for (i = 0; i < data.length; i++) {
    if (data[i].dateRounded == month) {
      ...
      for (var i = 0; i < 4; i++) {
        var radius = Math.sqrt(data[i].casesMax) / 4;
          var pathString = getPathString(i * 90, (i+1) * 90, radius);
          paper.path(pathString).attr({
              fill: col,
              opacity: 0.7,
